...
class MSW_Recent_Comments_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'msw_recent_comments', 'description' => __('Show recent comments with gravatar.', 'magazine-sidebar-widgets') );
	...
	function widget( $args, $instance ) {
		...
		$cache = wp_cache_get('widget_msw_recent_comments', 'widget');
		...
		$comments = get_comments( apply_filters( 'msw_widget_comments_args', array( 'number' => $number, 'status' => 'approve', 'post_status' => 'publish' ) ) );
		...
		if ( $comments ) {
			foreach ( (array) $comments as $comment) {
				...
				if ( $post_title == 1 ) :
					...
					$output .= __(' on', 'magazine-sidebar-widgets') . ' <a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>';
				else:
					$output .= '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_comment_author_link() . '</a>';

				endif;
				if ( $comment_content == 1 ) :
					$output .= __(' said:', 'magazine-sidebar-widgets') . ' <span class="comment-content">' . $this->widget_comment_length($comment->comment_content) . '</span> ';
				endif;
				if ( $comment_date == 1 ) :
		...
		wp_cache_set('widget_msw_recent_comments', $cache, 'widget');
	}
	...
	function flush_widget_cache() {
		wp_cache_delete('widget_msw_recent_comments', 'widget');
	}
...
?>
		...
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of comments to show:', 'magazine-sidebar-widgets'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('avatar'); ?>"><?php _e('Show avatar of comment author?', 'magazine-sidebar-widgets'); ?></label></p>
		...
		<label for="<?php echo $this->get_field_id('post_title'); ?>"><?php _e('Show post title of commented post?', 'magazine-sidebar-widgets'); ?></label></p>
		...
		<label for="<?php echo $this->get_field_id('comment_content'); ?>"><?php _e('Show excerpt of comment?', 'magazine-sidebar-widgets'); ?></label></p>
		
		<p><label for="<?php echo $this->get_field_id('comment_length'); ?>"><?php _e('Excerpt length in number of characters:', 'magazine-sidebar-widgets'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('comment_date'); ?>"><?php _e('Show date of comment?', 'magazine-sidebar-widgets'); ?></label></p>
